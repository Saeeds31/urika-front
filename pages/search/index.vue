<template>
  <v-container>
    ...
    <div class="search-box" v-motion-fade :duration="1200" :delay="600">
      <v-text-field :loading="isSearchTextInLoading" append-inner-icon="mdi-magnify" density="compact"
        label="نام کتاب یا نام نویسنده (حداقل سه کارکتر) ..." variant="solo" hide-details single-line
        @click:append-inner="onClick" v-model="searchText" v-on:keyup="trySearch"></v-text-field>
    </div>
    <div v-if="isSearched">
      <p class="mt-5">
        نتایج جستجو برای‌:
      ...
      <div class="search-result mt-3" v-if="pageModel?.items.length">
        <v-row dense>
          <v-col v-for="(card, index) in pageModel?.items" :key="card.title" :cols="card.flex ?? 6">
            <ProductCard v-motion-fade-visible :duration="700" :delay="index * 150 + 100" :product="card"
              :productType="getTypeByTypeId(card.type)"></ProductCard>
          </v-col>
        </v-row>
      </div>
      <v-empty-state v-else class="mt-5" icon="mdi-magnify"
        text="جستجوی شما نتیجه ای در پی نداشت درصورت نیاز میتوانید مجددا تلاش کنید."
        title="هیچ نتیجه ای یافت نشد!"></v-empty-state>
    </div>
  </v-container>
</template>
<script>
