  ...
  <v-container>
    <div class="d-flex mb-5 justify-space-between align-center">
      <div class="d-flex align-end opacity-90">
        <v-img v-motion-fade-visible :duration="1000" :delay="50" class="opacity-80" width="30" cover
          :src="pageIcon"></v-img>
        <h3 v-motion-fade-visible :duration="1050" :delay="50" class="font-weight-black mr-1">
          {{ pageTitle }}
        </h3>
      </div>
    </div>
    <v-row dense>
      <v-col v-for="(card, index) in pageModel.items" :key="card.title" :cols="card.flex ?? 6" :xs="card.xs ?? 6"
        :sm="card.sm ?? 6" :lg="card.md ?? 2">
        <ProductCard v-motion-fade-visible :duration="700" :delay="index * 150 + 100" :product="card"
          :productType="productType" :hideTypeTitle="true"></ProductCard>
      </v-col>
    </v-row>
  </v-container>
</template>
