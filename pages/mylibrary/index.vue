  ...
  <v-container>
    <div class="d-flex justify-space-between align-center">
      ...
      <div class="d-flex g-2">
        <p v-motion-fade :duration="750" :delay="300" @click="setItems('ordered')"
          :class="{ active: tab == 'ordered' }">
          خرید شده
        </p>
        <p v-motion-fade :duration="750" :delay="600" @click="setItems('liked')" :class="{ active: tab == 'liked' }">
          لایک شده
        </p>
      </div>
    </div>
    <div class="content" v-motion-fade :duration="750" :delay="1200">
      <div class="items mt-5" v-if="content.items.length > 0">
        <v-row dense>
          <v-col v-for="(card, index) in content.items" :key="card.title" :cols="card.flex ?? 6">
            <ProductCard v-motion-fade-visible :duration="700" :delay="index * 150 + 100" :product="card"
              :productType="getTypeByTypeId(card.type)" :hideFavorite="true"></ProductCard>
          </v-col>
        </v-row>
      </div>
